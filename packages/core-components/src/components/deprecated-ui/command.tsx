...
function CommandTextInput<T extends Data>(
	...
	return (
		<Animated.View
			entering={SlideInUp}
			className="relative z-50 flex-row items-center bg-background px-4 pb-4"
		>
			<Input
				role="searchbox"
				ref={inputRef}
			...
			>
				<FontAwesome6 name="x" size={24} className="text-muted-foreground" />
			</Button>
		</Animated.View>
	);
}
...
function CommandSectionList<T extends Data>(
	...
	return (
		<View
			style={{ minHeight: 2 }}
			className={cn("relative flex-1 overflow-hidden rounded-b-2xl", className)}
		>
			<View className="absolute left-0 right-0 top-0 h-14 bg-background" />
			<SectionList<T>
				ref={ref}
				data={data}
...
>(({ className, textClass, children, ...props }, ref) => {
	return (
		<Pressable
			ref={ref}
			className={cn(
				"border-t border-border bg-background px-5 pt-6",
				className,
			)}
			{...props}
		>
			{isTextChildren(children) ? (
				<Text
					className={cn(
						"px-0.5 text-base font-semibold text-muted-foreground",
						textClass,
					)}
				>
					{children}
	...
	return (
		...
		>
			<Button
				...
				className={cn("flex-1 flex-row items-center justify-start gap-3 px-2")}
